...
fn ev(parent: StateHash, id: u8, payload: u8, time: u64) -> EventEnvelope {
    EventEnvelope { event_id: [id; 16], parent_state_hash: parent, payload_hash: [payload; 32], logical_time: time }
}
...
fn test_invalid_parent_rejected() {
    ...
    assert!(matches!(eng.apply(&bad), Err(TransitionError::InvalidParent)));
}

#[test]
fn test_child_mutation_rejected() {
    let r1 = TransitionReceipt { parent_state_hash: [1;32], event_hash: [2;32], child_state_hash: [0;32], kernel_version: 1, kind: TransitionKind::ClaimCreated };
    let mut r2 = r1.clone(); r2.child_state_hash = [99;32];
    assert_ne!(r1.hash(), r2.hash());
}

#[test]
fn test_domain_separation() {
    let r = TransitionReceipt { parent_state_hash: [42;32], event_hash: [2;32], child_state_hash: [3;32], kernel_version: 1, kind: TransitionKind::ClaimVerified };
    let h = r.hash();
    assert_ne!(h, [42;32]);
    assert_ne!(h, [2;32]);
}
